...
describe("Store", function () {
    ...
    it("an action can create a new field", function () {
        ...
        const store = new S({});
        store.action1();
        assert.equal(store.state.field1, 1);
    });
    ...
    it("an action can change a field", function () {
        ...
        assert.equal(store.state.field1, 0);
        store.action1();
        assert.equal(store.state.field1, 1);
    });
    ...
    it("an action can set a field to undefined", function () {
        class S extends Store<{ field1?: number }> {
            action1() {
                this.action("action1", state => {
                    state.field1 = undefined;
                });
            }
        }
        const store = new S({field1: 1});
        store.action1();
        assert.isUndefined(store.state.field1);
    });
    ...
    it("data can be changed by subsequent actions", function () {
        ...
        assert.equal(store.state.field1, 1);
        store.action1();
        assert.equal(store.state.field1, 2);
    });
    ...
    it("action option - deepCloneFields", function () {
        ...
        const calls: any[] = [];
        store.select("field1").subscribe(s => calls.push(s.state.field1));
        store.action1();
        ...
        class S extends Store<{ field1: number, field2?: string, field3: number[]}> {
            action1() {
                this.action("a1", d1 => {
                    ...
                    assert.equal(this.state.field1, 1);
                    assert.equal(this.state.field2, "a");
                    ...
                    this.action("a2", d2 => {
                        ...
                        assert.equal(this.state.field1, 2);
                        assert.equal(this.state.field2, "b");
                        assert.deepEqual(this.state.field3, [99]);
        ...
        class S extends Store<{ field1?: number, field2?: number }> {
            action1() {
                this.action("action1", state => {
                    ...
                    afterAction: (store, state, changedFields, newFields) => {
                        assert.equal(state.field1, 1);
                        assert.equal(state.field2, 5);
